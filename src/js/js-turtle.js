/* THIS FILE IS EXTRACTED FROM A REPO AND MODIFIED AUTHOR CREDITS BELOW:
 *
 * original repo: https://github.com/hanumanum/js-turtle
 * original webpage(authors credits): https://hanumanum.github.io/js-turtle/
 *
 * 
 * edited by: abdedarghal111
 * repo: https://github.com/abdedarghal111/flor-para-otra-flor
*/
...
window.setJumpSteps = function(steps){
    jumpSteps = steps
}
...
window.forward = function(distance, step = false) {
    if (stepByStep && !step) {
        steps.push([window.forward, [distance]]);
    }
    else {
        ...
        while (distance > 0) {
            ...
            if (turtle.wrap) {
                ...
                else if (intersect(x, y, newX, newY, maxX, maxY, maxX, minY))
                    xWrap(maxX, minX);
                else if (intersect(x, y, newX, newY, minX, maxY, minX, minY))
                    xWrap(minX, maxX);
                else if (intersect(x, y, newX, newY, minX, maxY, maxX, maxY))
                    yWrap(maxY, minY);
                else if (intersect(x, y, newX, newY, minX, minY, maxX, minY))
                    yWrap(minY, maxY);
                else
...
window.showGrid = function(step) {
    let ws = turtle.canvas.x
    let hs = turtle.canvas.y
    angle(0);
    goto(12, 10);
    write("(0,0)");
    goto(ws - 11, 11);
...
window.angle = function(_angle, step = false) {
    if (stepByStep && !step) {
        steps.push([window.angle, [_angle]]);
    }
    else {
...
window.circle = function(radius, howMuchDrawn = 360, step = false) {
    if (stepByStep && !step) {
        steps.push([window.circle, [radius, howMuchDrawn]]);
    } else {
        howMuchDrawn = Math.min(howMuchDrawn, 360)
        howMuchDrawn = Math.max(howMuchDrawn, 0)
        let front = 2 * Math.PI * radius / 360
        let rotate = 1
        for (let i = 0; i < howMuchDrawn; i++) {
            forward(front, true)
            left(rotate, true)
        }
    }
...
//     let coords = canv.relMouseCoords(event);
//     let canvasX = coords.x;
//     let canvasY = coords.y;
//     co.innerHTML = "X:" + (canvasX - turtle.canvas.x / 2) + " Y:" + (-canvasY + turtle.canvas.y / 2);
...
function initialise(x, y) {
    ...
    imageCanvas.width = x;
    turtleCanvas.height = y;
    turtleCanvas.width = x;
...
function draw() {
    ...
    turtleContext.drawImage(imageCanvas, 0, 0, turtle.canvas.x, turtle.canvas.y, 0, 0, turtle.canvas.x, turtle.canvas.y);
}
